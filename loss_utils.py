...
def num_consistency_loss(anchors, neighbors):
    #  [cluster_num, batch_size]
    anchors = anchors.t()  # [cluster_num, batch_size]
    neighbors = neighbors.t()
    
    similarity = torch.sum(anchors * neighbors, dim=1)  # 直接求和替代bmm
    
    loss = F.binary_cross_entropy_with_logits(similarity, torch.ones_like(similarity))
    return loss

class DataContrastiveLoss(nn.Module):
    #our RC_loss:contrastive loss wigh data-level weight
    ...
    def forward(self, c_i, c_j,c_x,c_y,c_a,c_b,epoch,ifweights,ratio):
        """
        c_i/c_j/c_x/c_y/c_a/c_b: [batch_size, cluster_num]
        c_x,c_y/c_a,c_b:for data-level weight computation
        c_i,c_j:for contrastive loss computation
        ...
        # ================= sim =================
        features = torch.cat([c_i, c_j], dim=0)  # [2B, cluster_num]
        ...
        loss = -logits[torch.arange(2*batch_size), labels]  # [2B]

        #Weight-adjusted contrastive learning
        if ifweights==True:
        ...
        # weighted_loss= loss.sum()/N
        per_sample_loss = loss.view(-1, 2).sum(dim=1) 
